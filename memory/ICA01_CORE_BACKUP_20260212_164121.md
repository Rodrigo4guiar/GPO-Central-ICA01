...
<head>
    <meta charset="UTF-8">
    <title>GPO v2 // FINANCEIRO</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        :root { --neon: #00ffcc; --bg: #05080a; --panel: rgba(10, 25, 35, 0.9); --red: #ff003c; }
        ...
        input { width: 100%; padding: 12px; margin: 8px 0; border-radius: 5px; border: 1px solid #1a2a2a; background: #05080a; color: #fff; box-sizing: border-box; }
        ...
        button:hover { box-shadow: 0 0 20px var(--neon); transform: scale(1.02); }
        .btn-pay { background: #0a2a2a; color: var(--neon); border: 1px solid var(--neon); padding: 4px 10px; width: auto; font-size: 0.7rem; }
        .btn-delete { background: transparent; color: var(--red); border: 1px solid var(--red); width: auto; padding: 4px 10px; font-size: 0.7rem; }
        table { width: 100%; border-collapse: collapse; margin-top: 20px; font-size: 0.9rem; }
        th { text-align: left; color: var(--neon); border-bottom: 2px solid #1a2a2a; padding: 10px; }
        td { border-bottom: 1px solid #1a2a2a; padding: 10px; }
        .progress-container { background: #05080a; height: 12px; border-radius: 6px; overflow: hidden; border: 1px solid #1a2a2a; margin-top: 5px; }
...
<body>
    ...
    <div id="loginScreen">
        <div class="login-box">
            ...
            <p style="font-size: 0.7rem; margin-bottom: 20px; opacity: 0.6;">SISTEMA PROTEGIDO POR SOBERANIA 72%</p>
    ...
    <div id="mainContent">
        <div class="container">
            <header style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 40px; border-bottom: 1px solid var(--neon); padding-bottom: 10px;">
                <div>
            ...
            <div class="grid">
                <div class="panel">
                    ...
                    <input type="number" id="salarioInput" placeholder="Salário Mensal (R$)" onchange="updateSalario()">
                    
                    <h2 style="margin-top:40px;"><i class="fas fa-file-invoice-dollar"></i> Nova Despesa</h2>
                    <input type="text" id="nomeDespesa" placeholder="Nome da Conta (Ex: Carro)">
                    <input type="number" id="valorDespesa" placeholder="Valor Total (R$)">
                    <input type="number" id="parcelasTotal" placeholder="Qtd. Parcelas">
                    <input type="text" id="contatoNome" placeholder="Credor / Nome">
                    <input type="text" id="contatoInfo" placeholder="Contato / Pix / Tel">
                    <button onclick="adicionarDespesa()">Registrar no Sistema</button>
                </div>

                <div class="panel">
                    <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 20px;">
                        <div>
                            <span class="label">PROGRESSÃO DE QUITAÇÃO GLOBAL</span>
                            <div id="stats" class="stat-value">0.0%</div>
                        </div>
                        <div style="text-align: right;">
                            <span class="label">SALDO DISPONÍVEL</span>
                            <div id="saldoDisp" style="font-size: 1.2rem; color: #fff;">R$ 0,00</div>
                        </div>
                    </div>
                    
                    <div class="progress-container" style="height: 25px; margin-bottom: 30px;">
                        <div id="totalProgress" class="progress-fill" style="width: 0%"></div>
                    </div>

                    <h2><i class="fas fa-list"></i> Detalhamento de Passivos</h2>
    ...
    <script>
        // SEGURANÇA: Senha definida como '2026' por padrão. Mude abaixo se quiser.
        ...
        // LÓGICA DE DADOS
        let db = JSON.parse(localStorage.getItem('ica01_finance')) || { salario: 0, despesas: [] };

        function updateSalario() {
            db.salario = parseFloat(document.getElementById('salarioInput').value) || 0;
            saveAndRender();
        }
        ...
        function adicionarDespesa() {
            ...
            const pTot = parseInt(document.getElementById('parcelasTotal').value);
            if(!nome || !valor || !pTot) return alert("Preencha os campos obrigatórios.");

            db.despesas.push({
                id: Date.now(), nome: nome, valor: valor, pTotal: pTot, pPagas: 0,
        ...
        function pagarParcela(id) {
            ...
            if (item && item.pPagas < item.pTotal) {
                item.pPagas++;
                saveAndRender();
            }
        }
        ...
        function saveAndRender() {
            localStorage.setItem('ica01_finance', JSON.stringify(db));
            render();
        }

        function render() {
            if(sessionStorage.getItem('auth_ica01') !== 'true') return;
            document.getElementById('salarioInput').value = db.salario;
            const lista = document.getElementById('listaDespesas');
            lista.innerHTML = '';
            
            let totalPassivo = 0, totalPago = 0, parcelaMensalTotal = 0;

            db.despesas.forEach(d => {
                const perc = ((d.pPagas / d.pTotal) * 100).toFixed(0);
                const valorParcela = d.valor / d.pTotal;
                totalPassivo += d.valor;
                totalPago += (valorParcela * d.pPagas);
                if(d.pPagas < d.pTotal) parcelaMensalTotal += valorParcela;
                ...
                lista.innerHTML += `
                    <tr>
                        ...
                        <td style="font-size:0.7rem;">${d.cNome}<br>${d.cInfo}</td>
                        <td width="150">
                            ${perc}%
                            <div class="progress-container"><div class="progress-fill" style="width:${perc}%"></div></div>
                        </td>
                        <td>
                            <button class="btn-pay" onclick="pagarParcela(${d.id})">+ Pagar</button>
            ...
            const globalPerc = totalPassivo > 0 ? ((totalPago / totalPassivo) * 100).toFixed(1) : 0;
            document.getElementById('totalProgress').style.width = globalPerc + '%';
            document.getElementById('stats').innerText = globalPerc + '%';
            const saldo = db.salario - parcelaMensalTotal;
            document.getElementById('saldoDisp').innerText = saldo.toLocaleString('pt-br',{style:'currency',currency:'BRL'});
            document.getElementById('saldoDisp').style.color = saldo < 0 ? '#ff003c' : '#00ffcc';
        }

        window.onload = () => { if(sessionStorage.getItem('auth_ica01') === 'true') checkPass(); };
    </script>
</body>
</html>
